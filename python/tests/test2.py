...
class RSSMWorldModel(nn.Module, RLBase):
    n_rand_step = int(1e8)
    D_size = 1000

    T = 20
    n_opt_step = 1000
    loss_w = dict(z_enc=0.1, zp_dyn=1, rp=1, cp=1, xp=1)

    def __init__(s, env, seed=0):
        super().__init__()
        X, A, act_type = s.init(env, seed)
        s.z_nc = 16
        s.H = H = 64
        Z = s.z_nc**2
        s.twohot = TwoHot(64)
        ...
        s.test_pi = nn.Linear(H + Z, A)

        s.twohot.init(s.rew_hz_2_rp)
        s.opt = Adam(s.parameters())
    ...
    def losses(s, D: List[tc.Tensor]):
        ...
        return dict(
            ...
            cp=BCE_logits(cp_lg, tc.unsqueeze(c, -1)),
            xp=F.mse_loss(xp, x),
        )
...
if __name__ == "__main__":
    wm = RSSMWorldModel(env=gym.make("HalfCheetah-v5"))
    wm.dream()
